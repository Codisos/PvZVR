...
namespace Sisus.ComponentNames.EditorOnly
{
	...
	internal static class ComponentName
	{
		...
		internal static bool HasOverride([DisallowNull] Component component)
			=> overrides.ContainsKey(component);

		private static bool TryGetOverride([DisallowNull] Component component, out string nameOverride)
			=> overrides.TryGetValue(component, out nameOverride);

		[return: NotNull]
		internal static string Get([DisallowNull] Component component)
			=> TryGetOverride(component, out string nameOverride) ? nameOverride : GetDefault(component);
		...
		internal static string GetInspectorTitleAsPlainText([DisallowNull] Component component)
		{
		...
		internal static void ResetToDefault([DisallowNull] Component component, ModifyOptions modifyOptions)
		{
			...
			{
				changed |= UpdateNameContainer(component, modifyOptions);
			}
			...
			static bool UpdateNameContainer(Component component, ModifyOptions modifyOptions)
			{
				if(modifyOptions.IsDelayed())
				{
					EditorApplication.delayCall += ()=>
					{
						if(UpdateNameContainer(component, modifyOptions.Immediately()))
						{
							InspectorTitleChanged?.Invoke(component, "");
						}
					};
